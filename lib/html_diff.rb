module HtmlDiff
    ...
    DEFAULT_OPTIONS = {
    :tagclass_insert => 'diffmod',
    :tagclass_delete => 'diffmod',
    :tag_insert => 'ins',
    :tag_delete => 'del'
  }
  ...
  class DiffBuilder
    ...
    def merge_options(options)
      @tagclass_insert = options[:tagclass_insert]
      @tagclass_delete = options[:tagclass_delete]
      @tag_insert = options[:tag_insert]
      @tag_delete = options[:tag_delete]
    end
...
#          best_match_in_old, best_match_in_new, best_match_size, end_in_old, end_in_new)
    ...
    def replace(operation)
      delete(operation, @tagclass_delete)
      insert(operation, @tagclass_insert)
    end
    
    def insert(operation, tagclass = 'diffins')
      insert_tag(@tag_insert, tagclass, @new_words[operation.start_in_new...operation.end_in_new])
    end
    
    def delete(operation, tagclass = 'diffdel')
       insert_tag(@tag_delete, tagclass, @old_words[operation.start_in_old...operation.end_in_old])
    end
  ...
  def self.diff(a, b, options = {})
    DiffBuilder.new(a, b).build(DEFAULT_OPTIONS.merge(options))
  end
